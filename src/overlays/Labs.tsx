...
const PROJECTS_DATA = [
  {
    ...
    description:
      'React와 TypeScript를 기반으로 제작한 개인 포트폴리오 웹사이트입니다. Vite를 사용하여 빠르고 효율적인 개발 환경을 구축했으며, Tailwind CSS로 모던하고 반응형 UI를 구현했습니다. 연도별 경력, 기술 스택, 프로젝트 경험을 체계적으로 정리하였고, 화면 전환과 인터랙티브 오버레이 방식을 함께 적용해 부드러운 사용자 경험을 구현했습니다. 개발 과정 전반을 AI를 활용한 바이브 코딩 방식으로 진행하여, 세밀한 UI 디테일과 복잡한 인터랙션을 구현했습니다.',
  ...
  },
  {
    ...
    description:
      '서비스 소개와 신청 유도를 목적으로 제작한 마케팅 랜딩페이지로, 원페이지 스크롤 방식의 반응형 웹으로 구현되었습니다. 주요 섹션을 스크롤에 따라 순차적으로 노출되도록 구성하여 사용자가 자연스럽게 정보를 습득하고 행동으로 이어질 수 있도록 구현했습니다. 개발은 React기반으로 진행하였으며, 정적 페이지 형태로 빌드한 뒤 Vercel을 이용해 배포했습니다.',
  ...
  },
  {
    ...
    technologies: ['Java', 'Spring Boot', 'Expo', 'React Native', 'Redux'],
    description:
      'Spring Boot, React Native를 기반으로 개발한 등산 기록 및 관리 모바일 애플리케이션입니다. 사용자는 회원 가입 및 로그인 절차를 거쳐 서비스를 이용할 수 있습니다. 등산 기능을 실행하면 실시간으로 등산 진행 상황을 기록할 수 있으며, 등산 종료 후에는 경로와 기록이 저장됩니다. 또한 목표 산행을 달성하면 스탬프를 등록하는 게임화 요소가 포함되어 있습니다. React 학습 목적으로 구현해 보았고, 실제 개발 과정을 통해 React의 컴포넌트 구조와 상태 관리에 대해 직접 경험하고 익힐 수 있었습니다.',
